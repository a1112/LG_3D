"""External supervisor for the 3D algorithm process.

Threads cannot safely interrupt a native YOLO/OpenCV/Torch call.  This
supervisor therefore runs in a separate Python process and replaces the whole
algorithm process when its heartbeat is stale or reports an overdue activity.
"""
...
ALGORITHM_SCRIPT = APP_DIR / "main.py"
...
CHECK_INTERVAL = _positive_float_env("LG3D_ALGORITHM_3D_WATCHDOG_INTERVAL", 5.0)
STARTUP_GRACE = _positive_float_env("LG3D_ALGORITHM_3D_STARTUP_GRACE", 600.0)
HEARTBEAT_TIMEOUT = _positive_float_env(
    "LG3D_ALGORITHM_3D_HEARTBEAT_TIMEOUT", 30.0
)
FAILURE_THRESHOLD = _positive_int_env(
    "LG3D_ALGORITHM_3D_FAILURE_THRESHOLD", 3
)
RESTART_DELAY = _positive_float_env("LG3D_ALGORITHM_3D_RESTART_DELAY", 5.0)
RESTART_MAX_DELAY = _positive_float_env(
    "LG3D_ALGORITHM_3D_RESTART_MAX_DELAY", 300.0
)
RESTART_STABLE_SECONDS = _positive_float_env(
    "LG3D_ALGORITHM_3D_RESTART_STABLE_SECONDS", 300.0
)
STOP_TIMEOUT = _positive_float_env("LG3D_ALGORITHM_3D_STOP_TIMEOUT", 15.0)
MAX_FUTURE_SKEW = 300.0


def configure_logging() -> logging.Logger:
    log_dir = Path(os.getenv("LG3D_LOG_DIR", PROJECT_ROOT / "log")) / "Algorithm3D"
    ...
    result = logging.getLogger("algorithm_3d_watchdog")
...
def start_algorithm_process() -> subprocess.Popen:
    try:
        ...
        logger.warning("could not remove old 3D heartbeat: %s", exc)
    env = os.environ.copy()
    env[HEARTBEAT_FILE_ENV] = str(HEARTBEAT_FILE)
    env["LG3D_ALGORITHM_3D_WATCHDOG_CHILD"] = "1"
    logger.info("starting 3D algorithm service: script=%s", ALGORITHM_SCRIPT)
    kwargs = {
        "cwd": APP_DIR,
        "env": env,
    }
    ...
    if sys.platform == "win32":
        try:
            ...
            if result.returncode != 0:
                logger.error(
                    "taskkill failed for 3D algorithm pid=%s: %s",
                    process.pid,
                    (result.stderr or result.stdout).strip(),
                )
                process.kill()
        except (OSError, subprocess.TimeoutExpired) as exc:
            logger.error("3D process tree kill failed pid=%s: %s", process.pid, exc)
            process.kill()
        return
...
def stop_algorithm_process(process) -> None:
    if process.poll() is not None:
        return
    logger.warning("stopping unhealthy 3D algorithm service: pid=%s", process.pid)
    if sys.platform == "win32":
    ...
    try:
        ...
        logger.error(
            "3D algorithm did not stop in %.1fs; killing tree pid=%s",
            STOP_TIMEOUT,
            process.pid,
        )
        _force_kill_process_tree(process)
        try:
            ...
            logger.critical("3D algorithm remained alive after kill pid=%s", process.pid)
...
def acquire_instance_lock():
    ...
    handle = kernel32.CreateMutexW(None, False,
                                   "Global\\LG3D_Algorithm3D_Watchdog")
    if not handle:
        ...
        while True:
            if process is None:
                try:
                    ...
                    logger.exception(
                        "3D algorithm start failed: attempt=%s retry_in=%.1fs error=%s",
                        restart_failures,
                        delay,
                        exc,
                    )
            ...
            if exit_code is not None:
                ...
                delay = restart_backoff_seconds(restart_failures)
                logger.error(
                    "3D algorithm exited: pid=%s exit_code=%s uptime=%.1fs "
            ...
            logger.warning(
                "3D algorithm health failed: pid=%s failures=%s/%s reason=%s",
                process.pid,
            ...
            delay = restart_backoff_seconds(restart_failures)
            logger.error(
                "3D algorithm restart scheduled: attempt=%s retry_in=%.1fs",
                restart_failures,
                delay,
            )
            time.sleep(delay)
    except KeyboardInterrupt:
        logger.info("3D algorithm watchdog shutdown requested")
    finally:
...
if __name__ == "__main__":
    ...
    if instance_lock is None:
        logger.error("another 3D algorithm watchdog is already running; exiting")
    else:
        try:
