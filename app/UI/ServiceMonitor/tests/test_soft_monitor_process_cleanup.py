...
def test_target_pids_adopt_port_listener_and_watchdog_parent(monkeypatch):
    ...
    module = "lg3d_service_monitor.monitoring.services"
    monkeypatch.setattr(
        f"{module}.globSoftConfig.get_pid_list", lambda _: [])
    monkeypatch.setattr(f"{module}.getProcessesByTargets", lambda _: [])
    monkeypatch.setattr(f"{module}.get_listening_pids", lambda port: [101])
    monkeypatch.setattr(
        f"{module}.get_process_ids_from_files", lambda paths: [])
    observed = {}
    ...
    monkeypatch.setattr(f"{module}.get_process_tree_roots", fake_roots)
...
def test_target_pids_uses_persisted_worker_pid_files(monkeypatch):
    ...
    monkeypatch.setattr(
        f"{module}.globSoftConfig.get_pid_list", lambda _: [])
    monkeypatch.setattr(f"{module}.getProcessesByTargets", lambda _: [])
    monkeypatch.setattr(f"{module}.get_listening_pids", lambda port: [])
    monkeypatch.setattr(
        f"{module}.get_process_ids_from_files", lambda paths: [201])
    monkeypatch.setattr(
        f"{module}.get_process_tree_roots", lambda process_ids: [200])
