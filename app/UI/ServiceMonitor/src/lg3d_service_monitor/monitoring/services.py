...
from lg3d_service_monitor.config.service_registry import monitor_defaults
...
from lg3d_service_monitor.monitoring.state import globSoftConfig
from lg3d_service_monitor.platform.windows.processes import (
    get_listening_pids,
    get_process_ids_from_files,
    get_process_tree_roots,
    getProcessesByTargets,
    kill_process_and_children,
    normalize_path,
)


def try_get_int(value, default=5):
    try:
        return int(value)
...
class SoftMonitor(MonitorBase):
    ...
    def __init__(self, parent=None):
        ...
        log_folder = log_directory("SoftMonitor")
        super().__init__(config_f, log_folder, parent, default_data=defaults)
    ...
    def _resolve_exe_path(self, exe):
        ...
        if (exe_path.parent == Path(".")
                or exe_path.parts[0].casefold() == "launchers"):
            return str(launcher_directory() / exe_path.name)
        return str(resolve_lg3d_path(exe_path))

    def _read_only_blocked(self, action):
        if not is_read_only():
            return False
        self.log.warning(f"read-only mode: ignored {action}")
        return True
    ...
    readOnly = Property(bool, get_read_only, notify=readOnlyChanged)
    ...
    def _heartbeat_healthy(self, item):
        port = try_get_int(item.get("heartbeatPort", 0))
        if port <= 0:
            return True
    ...
    def _check_heartbeat(self, item):
        ...
        now = time.monotonic()
        timeout_seconds = max(
            try_get_int(item.get("heartbeatTimeoutSeconds",
        ...
        if self._read_only_blocked(f"heartbeat restart {name}"):
            return
    ...
    def stopExe(self, exe):
        if self._read_only_blocked(f"stop {exe}"):
            return False
    ...
    def _get_target_pids(self, normalized_exe, item=None):
        if item is None:
            item, _ = self._find_monitor_item(normalized_exe)
        pid_list = [
        ...
        if item:
            pid_list.extend(
                get_listening_pids(item.get("heartbeatPort", 0)))
            pid_list.extend(
                get_process_ids_from_files(
                    resolve_lg3d_path(path)
                    for path in item.get("processPidFiles", [])))
        return get_process_tree_roots(pid_list)

    def _stop_target(self, normalized_exe, name="", item=None):
        if item is None:
            item, _ = self._find_monitor_item(normalized_exe)
        stopped = False
        for _ in range(3):
            pid_list = self._get_target_pids(normalized_exe, item)
            if not pid_list:
                stopped = True
                break
            for pid in pid_list:
        ...
        remaining = self._get_target_pids(normalized_exe, item)
        if remaining:
    ...
    def startExe(self, name):
        if self._read_only_blocked(f"start {name}"):
            return False
    ...
    def _restart_exe_worker(self, item, exe):
        ...
        try:
            ...
            if not self._stop_target(exe, item_name, item):
                return
            time.sleep(1)
    ...
    def restartExe(self, name):
        if self._read_only_blocked(f"restart {name}"):
            return False
    ...
    @Slot(result=bool)
    def closeAll(self):
        if self._read_only_blocked("close all"):
            return False
        self.log.debug("全部关闭。")
        ok = True
        for item in list(self.monitorData):
            ...
            if not exe:
                continue
            self.manualStopped.add(exe)
            ok = self._stop_target(
                exe, item.get("name", ""), item) and ok
        return ok

    @Slot(result=bool)
    def startAll(self):
        if self._read_only_blocked("start all"):
            return False
        self.log.debug("全部启动。 ")
        started = False
        for item in list(self.monitorData):
            exe = normalize_path(self._resolve_exe_path(item["exe"]))
            if self._get_target_pids(exe, item):
                continue
            args = item["args"] if "args" in item else ""
    ...
    @Slot(result=bool)
    def restartAll(self):
        if self._read_only_blocked("restart all"):
            return False
        restart_items = []
        for item in list(self.monitorData):
            exe = normalize_path(self._resolve_exe_path(item.get("exe", "")))
            ...
            if heartbeat_failed_since is not None:
                ...
                issues.append({
                    ...
                    "message": (
                        ...
                        f"{max(try_get_int(item.get('heartbeatTimeoutSeconds', self.HEARTBEAT_RESTART_SECONDS)), 1)} seconds"
                    ),
                })
                continue
    ...
    def addApp(self, app: QtCore.QJsonValue):
        ...
        app["delay"] = try_get_int(app["delay"])
    ...
    def run(self):
        while self._running:
            for item in list(self.monitorData):
                ...
                args = item["args"] if "args" in item else ""
                delay = try_get_int(item["delay"]) if "delay" in item else 5
                monitor = item["monitorAble"] if "monitorAble" in item else True
                ...
                if not exe or not Path(exe_path).exists():
                    ...
                    continue
                try:
                    pid_list = self._get_target_pids(exe, item)
                except Exception as e:
                    self.log.error(f"获取进程列表失败 {name}: {e}")
                    continue
                if not pid_list:
