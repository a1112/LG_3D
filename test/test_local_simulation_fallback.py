...
def enable_test_mode(monkeypatch, available=True):
    monkeypatch.setenv("API_TESTDATA_COIL_ID", "125143")
    monkeypatch.setenv("API_TESTDATA_DIR", str(PROJECT_ROOT / "TestData" / "125143"))
    monkeypatch.delenv("API_DEVELOPER_MODE", raising=False)
    ...
    monkeypatch.setattr(ApiDataBase, "_test_mode_config_enabled", lambda: False)
...
def test_non_empty_database_result_keeps_shape_but_uses_testdata_content(monkeypatch):
    enable_test_mode(monkeypatch)
    ...
    assert result["Count"] == 1
    assert result["value"][0]["Id"] == 125143
    assert result["value"][0]["CoilNo"] == "125143"
